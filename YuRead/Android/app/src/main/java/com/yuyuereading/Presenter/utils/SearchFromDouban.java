...
public class SearchFromDouban {
     ...
     * 返回值：List<BookInfo>类
     */
    public static List<BookInfo> parsingBookInfoPerson(final JSONArray jsonArray) throws IOException {
        List<BookInfo> bookInfos = new ArrayList<>();
        for(i=0;i<jsonArray.size();i++){
            BookInfo bookInfo;
            bookInfo= BookInfoGetFromPerson.parsingBookInfo(jsonArray.get(i).toString());
            bookInfos.add(bookInfo);
        }
        return bookInfos;
    }
